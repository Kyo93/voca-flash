-- Migration 034: Fix hardcoded retention_rate in get_initial_app_data_v2
-- Previously returned 0.9 (hardcoded). Now computes real retention from review logs.
-- Uses same formula as get_user_progress_analytics: AVG(rating > 1) over last 30 days.
-- Returns 0 when no reviews exist (frontend handles "no data" display).
