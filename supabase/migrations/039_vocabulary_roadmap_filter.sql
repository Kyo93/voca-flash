-- Migration 039: Replace p_topic_id with p_roadmap_id in get_user_vocabulary_v2
-- The Kho từ vựng filter switched from per-topic to per-roadmap filtering.
-- A word matches a roadmap if it belongs to ANY topic of that roadmap.

DROP FUNCTION IF EXISTS get_user_vocabulary_v2(uuid, int, int, text, text, text, uuid, text, text);
...
BEGIN
    ...
    WITH filtered_records AS (
        ...
        WHERE s.user_id = p_user_id
          ...
          AND (
            p_roadmap_id IS NULL OR
            EXISTS (
                SELECT 1
                FROM topic_words tw
                JOIN topics t ON t.id = tw.topic_id
                WHERE tw.word_id = s.word_id AND t.roadmap_id = p_roadmap_id
            )
          )
          AND (
            p_stability = '' OR
