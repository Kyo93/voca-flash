...
describe('mobile typography density contract', () => {
  ...
  it('uses compact mobile spacing for the active Study focus route', () => {
    ...
    const activeSessionSource = source.slice(source.indexOf('data-mobile-study-session'))

    expect(source).toContain('data-mobile-study-session')
    expect(activeSessionSource).toContain('justify-start')
    expect(activeSessionSource).not.toContain('items-center justify-center px-4 pb-6 pt-4')
    expect(source).toContain('pt-4')
    expect(source).toContain('sm:pt-8')
    expect(source).toContain('pb-6')
    expect(source).toContain('sm:pb-12')
    expect(source).toContain('space-y-4 sm:space-y-8')
    expect(source).toContain('pt-1 sm:pt-8')
    expect(source).toContain('h-[min(49dvh,30rem)]')
  ...
  it('compacts flashcards and study actions on mobile without shrinking tap targets', () => {
    ...
    expect(backSource).toMatch(/p-5[\s\S]{0,80}sm:p-12/)
