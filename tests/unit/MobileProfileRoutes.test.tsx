...
describe('mobile Profile routes', () => {
  ...
  it('renders Settings as compact mobile controls with a safe save bar', () => {
    ...
    const saveBar = container.querySelector('[data-mobile-settings-savebar]')
    expect(saveBar).toBeTruthy()
    expect(saveBar?.className).not.toContain('fixed')
    expect(saveBar?.className).not.toContain('bottom-')
    expect(saveBar?.className).toContain('flex-col')
    expect(saveBar?.className).toContain('items-stretch')
    expect(screen.getByDisplayValue('Ocean')).toBeTruthy()
    expect(screen.getByRole('button', { name: /Save settings/i }).className).toContain('w-full')
  })
})
