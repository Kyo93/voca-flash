...
export default function LandingPage() {
  ...
  return (
    ...
    <div data-mobile-landing className="min-h-dvh overflow-x-hidden bg-surface text-secondary md:hidden">
      <header className="sticky top-0 z-30 border-b border-orange-100 bg-white px-5 pb-4 pt-[calc(env(safe-area-inset-top)+1rem)]">
        <nav className="flex items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-primary text-lg font-medium text-white">V</span>
            <span className="truncate text-xl font-medium text-primary">VocaFlash</span>
          </div>
          <a
            href="/login"
            className="inline-flex min-h-11 shrink-0 items-center justify-center rounded-full border border-primary/30 px-5 text-sm font-medium text-primary"
          >
            {t('landing.login')}
          </a>
        </nav>
      ...
      <main className="space-y-5 px-5 pb-[calc(env(safe-area-inset-bottom)+2rem)] pt-5">
        <section className="overflow-hidden rounded-3xl border border-orange-100 bg-white/80 p-5 shadow-sm">
          <span className="inline-flex items-center gap-2 rounded-full bg-orange-100 px-4 py-2 text-[0.7rem] font-medium uppercase text-primary">
            <span className="size-2 rounded-full bg-primary" />
            {t('landing.modernTechnique')}
          </span>
          <h1 className="mt-5 text-[2.25rem] font-medium leading-[1.08] text-secondary">
            {t('landing.heroTitle')}{' '}
          ...
          <div data-mobile-hero-actions className="mt-5 flex flex-col gap-3">
            <a
              href="/dashboard"
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-primary px-4 text-sm font-medium text-white shadow-md"
            >
            ...
            <a
              href="/login"
              className="inline-flex min-h-12 items-center justify-center rounded-2xl border border-secondary/10 bg-white px-4 text-sm font-medium text-secondary"
            >
              {t('landing.register')}
            </a>
          </div>
        ...
        <section className="overflow-hidden rounded-3xl border border-white bg-white shadow-sm">
          <div className="relative aspect-[4/3] overflow-hidden bg-orange-50">
            ...
            />
            <div className="absolute left-4 top-4 max-w-[70%] rounded-2xl bg-white p-3 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3 p-4">
            <div className="rounded-2xl bg-orange-50 p-4">
              ...
              <h2 className="mt-3 text-base font-medium text-secondary">{t('landing.feature1Title')}</h2>
            ...
            <div className="rounded-2xl bg-surface p-4">
              ...
              <h2 className="mt-3 text-base font-medium text-secondary">{t('landing.feature2Title')}</h2>
        ...
        <section className="rounded-3xl bg-secondary p-5 text-white shadow-sm">
          <h2 className="text-xl font-medium">{t('landing.ready')}</h2>
          <p className="mt-2 text-sm leading-6 text-white/80">{t('landing.ctaText')}</p>
          <div className="mt-5 flex flex-col gap-3">
            <a
              href="/dashboard"
              className="inline-flex min-h-12 items-center justify-center rounded-2xl bg-white px-4 text-sm font-medium text-primary"
            >
              {t('landing.studyNow')}
            </a>
            <a
              href="/login"
              className="inline-flex min-h-12 items-center justify-center rounded-2xl border border-white/30 px-4 text-sm font-medium text-white"
            >
              {t('landing.login')}
            </a>
          </div>
    ...
    <div data-desktop-landing className="relative hidden min-h-screen flex-col overflow-hidden bg-surface md:flex">
      ...
      <header className="w-full flex-none bg-white/60 backdrop-blur-xl border-b border-orange-100 z-50 relative">
        <nav className="flex items-center justify-between px-6 md:px-12 py-5 max-w-[1440px] mx-auto">
          <div className="text-3xl font-semibold tracking-tighter text-primary flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center gap-10">
            <a className="text-primary font-medium relative after:content-[''] after:absolute after:-bottom-1 after:left-0 after:w-full after:h-1 after:bg-primary after:rounded-full" href="#">{t('landing.articles')}</a>
          ...
          <div className="flex items-center gap-6">
            <a href="/login" className="px-8 py-2.5 rounded-full font-medium text-primary border-2 border-primary hover:bg-primary hover:text-white transition-all duration-300">{t('landing.register')}</a>
          </div>
        </nav>
      </header>
        ...
        <div className="grow flex items-stretch overflow-hidden max-w-[1440px] mx-auto w-full">
          ...
          <section className="w-[45%] flex flex-col p-12 md:p-16 overflow-y-auto">
            <div className="mb-auto">
              <span className="inline-flex items-center gap-2 px-5 py-2 mb-8 text-xs font-medium tracking-widest uppercase bg-orange-100 text-primary rounded-full shadow-sm">
                <span className="w-2 h-2 bg-primary rounded-full animate-pulse" />
                {t('landing.modernTechnique')}
              </span>
              <h1 className="text-5xl lg:text-6xl font-headline font-semibold leading-[1.1] tracking-tight text-secondary mb-8">
                {t('landing.heroTitle')}{' '}
                <span className="text-primary italic">{t('landing.heroTitleAccent')}</span>{' '}
                {t('landing.withSrs')}
              </h1>
              ...
              <div className="flex flex-wrap gap-5 mb-16">
                <a href="/dashboard" className="group px-10 py-4 hero-gradient text-white font-medium rounded-2xl shadow-[0_10px_30px_rgba(211,84,0,0.3)] hover:shadow-[0_15px_40px_rgba(211,84,0,0.5)] hover:-translate-y-1 active:scale-95 transition-all flex items-center gap-3">
                  {t('landing.startTrial')}
                  <span className="material-symbols-outlined text-xl group-hover:translate-x-1 transition-transform">arrow_forward</span>
                </a>
                <a href="/login" className="px-10 py-4 bg-white text-secondary font-medium rounded-2xl border-2 border-secondary/10 hover:border-primary/30 hover:bg-orange-50 active:scale-95 transition-all">
                  {t('landing.login')}
                </a>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 mt-auto">
              <div className="group bg-white p-6 rounded-2xl shadow-[0_8px_20px_rgba(0,0,0,0.04)] border border-orange-50 hover:border-primary/20 hover:shadow-xl transition-all duration-300">
                ...
                <h3 className="text-lg font-semibold text-secondary mb-2">{t('landing.feature1Title')}</h3>
              ...
              <div className="group bg-white p-6 rounded-2xl shadow-[0_8px_20px_rgba(0,0,0,0.04)] border border-orange-50 hover:border-primary/20 hover:shadow-xl transition-all duration-300">
                ...
                <h3 className="text-lg font-semibold text-secondary mb-2">{t('landing.feature2Title')}</h3>
            ...
            <div className="grid grid-rows-[auto_1fr_auto] gap-12 h-full relative z-10">
                  ...
                  <div className="absolute top-10 left-10 animate-float">
                    <div className="bg-white/90 backdrop-blur p-4 rounded-2xl shadow-xl flex items-center gap-3">
                      ...
                      <div className="text-xs">
                        <div className="font-medium">{t('landing.floatingCard.mastered')}</div>
              ...
              <div className="space-y-10">
                <div className="flex items-center gap-4">
                  ...
                  <h2 className="text-3xl font-semibold text-secondary">{t('landing.solutionTitle')}</h2>
                </div>
                <div className="space-y-6">
                  <div className="group flex items-start gap-6 p-6 bg-white/60 rounded-2xl hover:bg-white transition-all shadow-sm hover:shadow-md border border-transparent hover:border-orange-100">
                    <span className="text-5xl font-semibold text-orange-200 group-hover:text-primary/20 transition-colors leading-none">01</span>
                    <div>
                      <h4 className="text-xl font-medium text-secondary mb-2 group-hover:text-primary transition-colors">{t('landing.forAllAges')}</h4>
                      <p className="text-on-surface-variant">{t('landing.forAllAgesDesc')}</p>
                    </div>
                  </div>
                  <div className="group flex items-start gap-6 p-6 bg-white/60 rounded-2xl hover:bg-white transition-all shadow-sm hover:shadow-md border border-transparent hover:border-orange-100">
                    <span className="text-5xl font-semibold text-orange-200 group-hover:text-primary/20 transition-colors leading-none">02</span>
                    <div>
                      <h4 className="text-xl font-medium text-secondary mb-2 group-hover:text-primary transition-colors">{t('landing.parentMode')}</h4>
              ...
              <div className="mt-auto relative group">
                ...
                <div className="relative hero-gradient rounded-4xl p-8 md:p-10 overflow-hidden flex flex-col lg:flex-row items-center justify-between shadow-2xl gap-8">
                  <div className="relative z-10 lg:max-w-[60%] text-center lg:text-left">
                    <h2 className="text-3xl font-semibold text-white mb-3">{t('landing.ready')}</h2>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 relative z-10 shrink-0">
                    <a href="/dashboard" className="px-8 py-3 bg-white text-primary text-base font-semibold rounded-xl hover:scale-105 hover:bg-orange-50 transition-all shadow-lg">
                      {t('landing.studyNow')}
                    </a>
                    <button className="px-8 py-3 border-2 border-white/40 text-white text-base font-medium rounded-xl hover:bg-white/10 transition-all">
                      {t('landing.pricing')}
                    </button>
                  </div>
      ...
      <footer className="w-full flex-none bg-white border-t border-orange-50 relative z-20">
        <div className="flex flex-col md:flex-row items-center justify-between px-6 md:px-12 py-8 max-w-[1440px] mx-auto gap-6 md:gap-0">
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-10 text-center md:text-left">
            <span className="font-semibold text-primary text-xl">VocaFlash</span>
            <span className="text-sm text-on-surface-variant/70">{t('landing.footer.copyright')}</span>
          </div>
          <div className="flex flex-wrap justify-center gap-6 md:gap-10 text-sm font-medium text-secondary">
