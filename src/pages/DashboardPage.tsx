    ...
    <div className="relative flex-1 px-10 py-8" style={{ maxWidth: LAYOUT_TOKENS.MAX_WIDTH, margin: '0 auto', width: '100%' }}>
      ...
      <div className="mt-12">
        <div className="flex justify-between items-baseline mb-8">
          <div>
            <h4 className="text-3xl font-semibold text-on-surface tracking-tight">{t('home.continueLearning')}</h4>
            <div className="flex items-center gap-2 mt-2">
               <span className="w-8 h-px bg-primary/20" />
               <p className="text-stone-400 font-medium text-sm italic">{t('home.continueDesc')}</p>
            </div>
          </div>
          <Link className="text-primary font-semibold text-[11px] uppercase tracking-widest hover:text-secondary transition-all" to="/library">{t('home.viewLibrary')}</Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {card1 ? (
            <div className={`topic-card group bg-surface-container rounded-4xl sun-drenched-shadow overflow-hidden hover:bg-surface-container-lowest transition-all duration-500`}>
              <div className="relative h-56 overflow-hidden">
                ...
                <span className={`absolute bottom-4 left-4 px-3 py-1 bg-primary text-white text-[9px] font-semibold ${DESIGN_TOKENS.RADIUS.LG} uppercase tracking-widest`}>
                  {isResume ? t('home.tracking') : t('home.suggestion')}
                </span>
              </div>
              <div className="p-6">
                <h5 className="text-xl font-semibold text-on-surface mb-2">{card1.name}</h5>
                <p className="text-sm text-stone-400 mb-5 line-clamp-2">{card1.description || t('home.continueLearning')}</p>
                <div className="flex justify-between items-center">
                  <span className="text-stone-300 text-[10px] font-semibold uppercase tracking-[0.2em]">
                    {isResume ? t('home.unfinished') : t('home.newModule')}
                  </span>
                  <Link
                    to={`/study?topic=${card1.slug}&topicId=${card1.id}&roadmapId=${card1.roadmap_id}`}
                    className="text-secondary text-[11px] font-semibold uppercase tracking-wider flex items-center gap-1 hover:underline underline-offset-2"
                  >
          ...
          <div className={`group bg-surface-container rounded-4xl sun-drenched-shadow overflow-hidden hover:bg-surface-container-lowest transition-all duration-500`}>
            <div className="relative h-56 overflow-hidden bg-secondary/10 flex items-center justify-center">
              ...
              <span className={`absolute bottom-6 left-6 px-4 py-1.5 bg-secondary text-white text-[9px] font-semibold rounded-lg uppercase tracking-[0.2em]`}>{t('home.activeRecall')}</span>
            </div>
            <div className="p-6">
              <h5 className="text-xl font-semibold text-on-surface mb-2">{t('home.maintenanceTitle')}</h5>
              <p className="text-sm text-stone-400 mb-5 line-clamp-2">{t('home.maintenanceDesc')}</p>
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-secondary animate-pulse" />
                  <span className="text-secondary text-[11px] font-semibold uppercase tracking-wider">
                    {loading ? '...' : t('home.wordsDue', { count: reviewCount })}
                  </span>
                </div>
                <Link to="/review" className="text-secondary text-[11px] font-semibold uppercase tracking-wider flex items-center gap-1 hover:underline underline-offset-2">
