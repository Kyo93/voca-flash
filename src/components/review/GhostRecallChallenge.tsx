...
export default memo(function GhostRecallChallengeInner({ word, onSubmit }: GhostRecallChallengeProps) {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      {/* Header */}
      <div className="mb-4 flex w-full flex-col gap-3 rounded-2xl bg-surface-container-low p-4 sm:mb-8 sm:gap-6 sm:p-6">
        ...
        <div className="flex justify-center">
          ...
          >
            {showHint ? (
              <div className="text-center font-headline text-3xl font-semibold text-on-surface transition-all duration-300 sm:text-4xl">
                {word.definition}
              </div>
            ) : (
              <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-1.5">
                <span className="material-symbols-outlined text-2xl text-primary/30">visibility_off</span>
                <span className="text-[9px] font-medium uppercase tracking-widest text-outline">{t('arena.holdToReveal')}</span>
              </div>
            )}
            {!showHint && (
              <div className="flex min-h-11 items-center justify-center gap-2" aria-hidden="true">
                {Array.from({ length: Math.min(Math.max(word.definition.length, 4), 10) }).map((_, index) => (
                  <span key={index} className="h-3 w-3 rounded-full bg-primary/15" />
                ))}
              </div>
            )}
          </div>
      ...
      <button
        ...
        className="mb-4 mt-3 min-h-11 text-[10px] font-medium uppercase tracking-widest text-outline transition-colors hover:text-primary sm:mb-12 sm:mt-6"
      >
