      ...
      <div className="flex items-center gap-4 p-4 bg-stone-50 rounded-xl flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-secondary">
            {t('admin.import.newWords', { count: stats.ok })}
          </span>
        </div>
        {stats.duplicates > 0 && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-secondary">
              {t('admin.import.duplicates', { count: stats.duplicates })}
            </span>
        ...
        {stats.errors > 0 && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-secondary">
              {t('admin.import.errorsCount', { count: stats.errors })}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="border border-stone-200 rounded-xl overflow-hidden overflow-x-auto">
        <table className="w-full min-w-[1200px]">
          <thead className="bg-stone-50">
            <tr>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase w-8">#</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase w-36">{t('admin.import.tableHeader.word')}</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase w-12">{t('admin.import.tableHeader.type')}</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase w-14">{t('admin.import.tableHeader.diff')}</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase w-32">{t('admin.import.tableHeader.topics')}</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase min-w-[180px]">{t('admin.import.tableHeader.definition')}</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase min-w-[150px]">{t('admin.import.tableHeader.exampleEn')}</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase min-w-[130px]">{t('admin.import.tableHeader.exampleVi')}</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase w-32">{t('admin.import.tableHeader.wrongChoices')}</th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-stone-400 uppercase w-10">{t('admin.import.tableHeader.status')}</th>
            </tr>
          </thead>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <td className="px-2 py-2">
                  <p className="font-medium text-secondary text-sm leading-tight truncate" title={row.word}>
                    {row.word}
                  </p>
                  {row.phonetic && (
                ...
                <td className="px-2 py-2">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {row.unmatchedTopics?.map(un => (
                      <span key={un} className="text-xs font-medium px-1.5 py-0.5 rounded-full bg-yellow-100 text-yellow-700 whitespace-nowrap">
                        ⚠️ {un}
                      </span>
                    ))}
                  </div>
                ...
                <td className="px-2 py-2">
                  {row.status === 'invalid' ? (
                    <div>
                      <span className="text-xs font-medium text-red-600">❌ {t('admin.import.status.error')}</span>
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-medium text-yellow-600">⚠️ {t('admin.import.status.duplicate')}</span>
                      <select
                      ...
                      >
                        <option value="skip">{t('admin.import.action.skip')}</option>
                        <option value="update">{t('admin.import.action.update')}</option>
                      </select>
                    </div>
                  ) : (
                    <span className="text-xs font-medium text-green-600">✓ {t('admin.import.status.ok')}</span>
                  )}
                </td>
              </tr>
            ))}
