        ...
        <table className="w-full min-w-[1200px]">
          <thead className="bg-stone-50">
            <tr>
              ...
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase w-36">{t('admin.import.tableHeader.word')}</th>
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase w-12">{t('admin.import.tableHeader.type')}</th>
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase w-14">{t('admin.import.tableHeader.diff')}</th>
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase w-32">{t('admin.import.tableHeader.topics')}</th>
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase min-w-[180px]">{t('admin.import.tableHeader.definition')}</th>
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase min-w-[150px]">{t('admin.import.tableHeader.exampleEn')}</th>
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase min-w-[130px]">{t('admin.import.tableHeader.exampleVi')}</th>
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase w-32">{t('admin.import.tableHeader.wrongChoices')}</th>
              <th className="px-2 py-2 text-left text-xs font-black text-stone-400 uppercase w-10">{t('admin.import.tableHeader.status')}</th>
            </tr>
          </thead>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <td className="px-2 py-2">
                  {row.status === 'invalid' ? (
                    <div>
                      <span className="text-xs font-bold text-red-600">❌ {t('admin.import.status.error')}</span>
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-bold text-yellow-600">⚠️ {t('admin.import.status.duplicate')}</span>
                      <select
                    ...
                    <span className="text-xs font-bold text-green-600">✓ {t('admin.import.status.ok')}</span>
                  )}
                </td>
              </tr>
            ))}
