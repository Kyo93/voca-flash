...
export default function MobileProgressView({
  ...
  return (
    <main data-mobile-progress className="min-h-full bg-surface px-4 pb-6 pt-3">
      <section className="rounded-3xl bg-surface-container-lowest p-4 shadow-sm ring-1 ring-outline-variant/30">
        ...
        <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl bg-primary-container/65 p-3 text-on-primary-container">
          <div className="min-w-0">
            ...
            <p className="mt-1 truncate text-lg font-bold">{userLevel}</p>
            <p className="mt-0.5 truncate text-xs font-medium opacity-75">{t(greetingKey)}</p>
          </div>
          {reviewCount > 0 && (
            <Link
              to="/review"
              className="flex min-h-11 shrink-0 items-center justify-center gap-1 rounded-2xl bg-primary px-3 text-xs font-bold text-on-primary active:scale-95"
            >
        ...
        <div className="grid grid-cols-3 gap-2">
          {links.map(link => (
            <Link
              key={link.to}
              to={link.to}
              className="flex min-h-20 flex-col items-center justify-center gap-2 rounded-2xl bg-surface-container-low p-3 text-center text-xs font-bold text-on-surface-variant active:scale-95"
            >
        ...
        {metrics.map(metric => (
          <article key={metric.key} className="rounded-2xl bg-surface-container-lowest p-3 shadow-sm ring-1 ring-outline-variant/30">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="truncate text-[10px] font-bold uppercase tracking-tight text-on-surface-variant/65">{metric.hint}</span>
            </div>
            <p className="mt-3 text-xl font-bold tabular-nums text-on-surface">{metric.value}</p>
      ...
      <section className="mt-4 rounded-3xl bg-secondary text-on-secondary p-4">
        <p className="text-[11px] font-bold uppercase tracking-wider opacity-75">{t('profileMobile.nextBadge')}</p>
        <p className="mt-1 text-lg font-bold leading-tight">{nextBadge}</p>
        {rewardProgress && (
          <div className="mt-4">
            ...
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-on-secondary/20">
      ...
      <section className="mt-4 rounded-3xl bg-surface-container-lowest p-4 shadow-sm ring-1 ring-outline-variant/30">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-base font-bold text-on-surface">{t('progress.weak_clusters')}</h2>
          </div>
          <Link to="/review" className="text-xs font-bold text-primary">
        ...
        <div className="mt-3 space-y-2">
          {weakWords.length === 0 ? (
            ...
            <div key={word.id} className="flex items-center justify-between gap-3 rounded-2xl bg-surface-container-low p-3">
              <div className="min-w-0">
                <p className="truncate text-sm font-bold text-on-surface">{word.word}</p>
                <p className="truncate text-xs font-medium text-on-surface-variant">{word.meaning}</p>
              </div>
              <span className="rounded-full bg-primary-container px-2 py-1 text-[11px] font-bold text-primary">
                {word.fail_count}
              </span>
            </div>
          ))}
