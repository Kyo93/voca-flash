...
export default function AdminSidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full flex flex-col p-4 gap-2 bg-stone-50 w-64 z-50 border-r border-stone-100">
      {/* Logo */}
      <div className="flex items-center gap-3 mb-8 px-2">
        <div className="w-10 h-10 bg-gradient-to-br from-orange-600 to-orange-800 rounded-lg flex items-center justify-center text-white shadow-lg">
          <span className="material-symbols-outlined-filled text-xl">admin_panel_settings</span>
        </div>
        <div>
          <h1 className="text-xl font-black text-primary leading-none">Admin Panel</h1>
          <p className="text-[10px] text-stone-500 font-medium tracking-widest uppercase">VocaFlash CMS</p>
        </div>
      </div>

      {/* Admin Navigation */}
      <nav className="flex flex-col gap-1">
        <p className="px-4 py-2 text-[10px] font-black text-stone-400 tracking-widest uppercase">Quản lý</p>
        {navItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
      ...
      <Link
        to="/dashboard"
        className="flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm text-stone-500 hover:text-stone-700 hover:bg-stone-100"
      >
