      ...
      <Link {...commonProps} className={`${commonProps.className} topic-card md:col-span-2 relative group overflow-hidden rounded-5xl bg-surface-container-highest min-h-[400px] flex items-end shadow-[0_40px_60px_-10px_rgba(113,55,0,0.06)]`}>
        ...
        <div className="relative p-10 w-full space-y-5">
          <div className="space-y-2">
            <span className="text-xs uppercase tracking-[0.2em] font-medium text-primary-fixed">{t('roadmap.card.activeChapter')}</span>
            <h2 className="text-4xl font-medium text-white">{t('roadmap.card.topicPrefix')}: {topic.name}</h2>
            <div className="flex items-center gap-3">
              <span className="px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm text-[10px] font-medium text-white/90 border border-white/10">
                {t('topics.wordsCount', { count: stats.total })}
              </span>
              {isCompleted && (
                <span className="px-3 py-1 rounded-full bg-green-500/20 backdrop-blur-sm text-[10px] font-medium text-green-400 border border-green-500/20 flex items-center gap-1">
          ...
          <button className="bg-linear-to-r from-primary to-primary-container text-white px-10 py-5 rounded-full font-medium text-lg shadow-xl hover:shadow-primary/20 transition-all active:scale-95 flex items-center gap-3 w-fit">
      ...
      <Link {...commonProps} className={`${commonProps.className} topic-card relative group overflow-hidden rounded-4xl bg-surface-container-low min-h-[340px] flex flex-col justify-between border-b-4 border-secondary/20 shadow-lg shadow-secondary/5`}>
        ...
        <div className="relative p-8 space-y-6 z-10">
          <div className="flex justify-between items-start">
            <span className="bg-secondary text-white px-4 py-1.5 rounded-full text-[10px] font-semibold uppercase tracking-widest">{t('roadmap.card.upNext')}</span>
            <span className="material-symbols-outlined text-white/80">trending_up</span>
          </div>
          <div className="space-y-2">
            <h3 className="text-3xl font-medium text-white">{t('roadmap.card.topicPrefix')}: {topic.name}</h3>
            <span className="inline-block px-2 py-0.5 rounded bg-white/10 backdrop-blur-sm text-[9px] font-medium text-white/80 border border-white/10 uppercase tracking-wider">
              {t('topics.wordsCount', { count: stats.total })}
            </span>
            {topic.description && (
    ...
    >
      <div className="flex justify-between items-start">
        ...
        {isCompleted ? (
          ...
          <span 
            className="text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded opacity-60"
      ...
      <div className="mt-6">
        <h4 className="text-xl font-medium mb-1" style={{ color: theme.text }}>{topic.name}</h4>
        <div className="flex flex-col gap-1.5">
          ...
          <span className="text-[10px] font-medium opacity-50 uppercase tracking-widest" style={{ color: theme.text }}>
            {t('topics.wordsCount', { count: stats.total })}
          </span>
        </div>
      </div>
