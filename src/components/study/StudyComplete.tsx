...
export default function StudyComplete({ total }: StudyCompleteProps) {
  ...
  return (
    <div data-mobile-study-complete className="flex min-h-[100dvh] flex-col items-center justify-center px-4 py-6 text-center">
      ...
      <h2 className="mb-3 text-2xl font-extrabold tracking-tight text-on-surface sm:mb-4 sm:text-4xl sm:font-semibold">{t('studyComplete.title')}</h2>
      ...
      <div className="flex w-full max-w-sm flex-col justify-center gap-3 sm:flex-row sm:gap-4">
        <Link
          to="/dashboard"
          className="flex min-h-12 flex-1 items-center justify-center rounded-xl bg-surface-container-highest px-6 py-3 text-center font-medium text-on-surface shadow-md transition-all hover:bg-surface-container-high active:scale-95 sm:px-8 sm:py-4"
        >
          {t('studyComplete.dashboard')}
        </Link>
        <Link
          to={studyLink}
          className="flex min-h-12 flex-1 items-center justify-center rounded-xl bg-primary px-6 py-3 text-center font-medium text-white shadow-lg transition-all hover:brightness-110 active:scale-95 sm:px-8 sm:py-4"
        >
