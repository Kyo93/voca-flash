    ...
    <div className="activity-heatmap bg-white p-8 rounded-4xl border border-stone-100 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[11px] font-black text-stone-400 uppercase tracking-[0.2em]">{t('progress.activityFrequency')}</h3>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-stone-300 font-black uppercase tracking-widest">{t('progress.less')}</span>
          ...
          <span className="text-[10px] text-stone-300 font-black uppercase tracking-widest">{t('progress.more')}</span>
        </div>
      </div>
      ...
      <div className="grid grid-flow-col grid-rows-7 gap-1.5 h-36">
        {days.map((day, i) => (
          <div
            key={i}
            className={`w-full h-full rounded-xs transition-all duration-700 cursor-help ${day.level === 0 ? 'bg-stone-50 border border-stone-100/50' :
                day.level === 1 ? 'bg-orange-100/40' :
                  day.level === 2 ? 'bg-orange-300/60' :
                    'bg-primary shadow-[0_0_12px_rgba(211,84,0,0.25)] scale-105 z-10'
              }`}
            title={t('progress.intensity', { level: day.level })}
          ></div>
        ))}
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        <p className="text-[10px] text-stone-400 font-bold italic opacity-60">
          {t('progress.streakNote', { count: streakDays })}
        </p>
        <div className="flex gap-4">
          <div className="flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full bg-orange-400"></div>
            <span className="text-[9px] font-black text-stone-500 uppercase tracking-widest">{t('progress.maintain')}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse"></div>
            <span className="text-[9px] font-black text-stone-500 uppercase tracking-widest">{t('progress.peak')}</span>
          </div>
        </div>
      </div>
    </div>
